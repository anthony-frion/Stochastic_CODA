...
class LightningBaseModel(pl.LightningModule):
    ...
    def do_step(self, batch: torch.Tensor, stage: str = "Training") -> torch.Tensor:
        ...
        mu_left, sigma_left = self.assimilation_network.forward(feed_forward_left)
        mu_right, sigma_right = self.assimilation_network.forward(feed_forward_right)
        if self.sample_mean:
            estimated_state_left = mu_left
            estimated_state_right = mu_right
        else:
            estimated_state_left = sample_from_diag_gaussian(mu_left, sigma_left)
            estimated_state_right = sample_from_diag_gaussian(mu_right, sigma_right)
        rollout = self.rollout(estimated_state_left)

        loss_dict: dict = self.loss_function(
            prediction=[rollout, rollout[:, -1, :].unsqueeze(1)],
            target=[observations_data, mu_right, sigma_right],
